...
_![][image18]Frame’s image ingestion pipeline architecture with specialized components for validation, pre-processing, embedding, and storage_
...
_![][image19]Frame’s search system architecture with text and image search inputs processed through validation, pre-processing, and embedding. The query embedding is used to find similar vectors stored in the database_
...
_![][image20]Image validation workflow: Request flows through API Gateway and Lambda to SQS. Immediate response to users while processing continues asynchronously_
...
_![][image21]Image pre-processing workflow: Image → buffer → resizing → base64_
...
_![][image22]Embedding generation: Lambda functions for both ingestion and search connect to Amazon Bedrock, which produces vector embeddings_
...
_![][image23]Embeddings stored in PostgreSQL with the pgvector extension_
...
_![][image19]Search Operation Flow: The S3 bucket provides temporary storage for uploaded local image files_
...
_![][image24]Recommendation Operation Flow: Direct retrieval of similar images using existing embeddings, bypassing pre-processing and embedding generation_
...
_![][image25]Image Management Operation Flow: Fetch/delete operations connect directly to the database from the API_
...
_![][image26]Frame’s SDK connects a user’s application to the Frame infrastructure. Six SDK methods map to Frame API endpoints_
